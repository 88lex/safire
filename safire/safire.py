...
class List(Help):
    ...
    def sas(self, filter="", file_tag="", token=token, prt=True):
        ...
        for project in sorted(projId_list):
            try:
                ...
                svcacctlist += resp
                sa_summary += [f"  {len(resp)} service accounts (SAs) found in project: {project}"]
            except:
                sa_summary += [f"  No service accounts (SAs) found in project: {project}"]
                continue
        svcacctlist.sort(key=lambda x: x.get('email'))
        if not prt:
            return [i['email'] for i in svcacctlist], [i['uniqueId'] for i in svcacctlist]
        if svcacctlist:
        ...
        for group in group_list:
            try:
                ...
                while 'nextPageToken' in resp:
                    resp = svc.members().list(groupKey=group, pageToken=resp['nextPageToken']).execute()
    ...
    def jsons(self, sa_path=sa_path, filter="", file_tag="", prt=True):
        ...
        if prt and json_keys:
            self._export(json_keys, filter, file_tag, ["client_email"], 'json_keys', prt)
...
class Add(Help):
    ...
    def projects(self,
        ...
        for _ in range(num_new_projects):
            ...
            batch.add(cloud.projects().create(body={'project_id': new_proj, 'name': new_proj}))
            next_project_num += 1
            print(f"Creating the project: {new_proj}")
        batch.execute()
        print('\nCreated the following projects:', *new_projs, sep='\n')
        print("Sleep briefly to allow backend to register projects. Then enabling services.\n")
        sleep(10)
    ...
    def apis(self, filter="", svcs_to_enable=svcs_to_enable, token=token, prt=False):
        ...
        for project in projId_list:
            for svc1 in svcs_to_enable:
                ...
                batch.add(svcusage.services().enable(name=f"projects/{project}/services/{svc1}"))
            batch.execute()
    ...
    def jsons(self,
        ...
        for project in sorted(projId_list):
            ...
            for i in resp:
                if i is not None:
                    k = (i["name"][i["name"].rfind("/"):], b64decode(i["privateKeyData"]).decode("utf-8"))[1]
    ...
    def members(self, project_filter, group_filter, retry=5, file_tag="", group_token=group_token, prt=False):
        ...
        for group in group_list:
            for project in projId_list:
                ...
                if len(add_sas) == 0:
                    continue
                print("Adding", str(len(add_sas)), "service accounts to group:", group)
                while add_sas and retry:
                    retry -= 1
                    for email in add_sas:
                        batch.add(admin.members().insert(groupKey=group, body={"email": email, "role": "MEMBER"}))
                    batch.execute()
                    group_members = self._list.members(group_filter, file_tag, group_token, prt)
                    add_sas = [i for i in sa_emails if i not in group_members]
            print(f"{len(sa_emails)-len(add_sas)} SA emails from {project} are in {group}. {len(add_sas)} failed.")
    ...
    def members(self, group_filter, retry=5, batch_size=100, file_tag="", group_token=group_token, prt=False):
        ...
        for group in group_list:
            ...
            while len(group_members) > 1 and retry:
                print(f"Removing {str(len(group_members))} SAs from group: {group}. Batch = {batch_size}")
                retry -= 1
...
class Rename:
    ...
    def jsons(self, rename_type, dir=f"{sa_path}/"):
        ...
        new_name = []
        if rename_type == 'seq':
            [os.rename(dir + file, dir + f"{i+1}.json") for i, file in enumerate(sorted(filelist))]
        else:
